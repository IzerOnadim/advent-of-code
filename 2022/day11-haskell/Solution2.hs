...
monkeyBusiness = product . take 2 . traceShowId . reverse . sort . inspections . runRounds 10000
...
apply idx (M items op test t f i) ms = newMs//[(idx, M [] op test t f (i + length items))]
  where
    ...
    inspect :: Op -> Int -> Int -> Int
    inspect op md item = mod (exec op item) md
