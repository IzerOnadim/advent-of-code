...
numVisited = length . visited (replicate 10 (0, 0)) []
...
move _    0     r           = r
move next steps ((x:xs), v) = move next (steps - 1) (newS, ((last newS) : v))
  where newS = moveOnce ((next x) : xs)

moveOnce :: State -> State
moveOnce (x:y:xs) = (x : moveOnce ((follow x y) : xs))
moveOnce x        = x
